...
var rhoMkSort = c.fun({ x: c.number }).returns(c.number).wrap(function (x) {
        var arr = [];
        for (var i = 0; i < x; i++) {
            arr.push(i);
        }
...
var inner_mkSort = _c$600.fun([typeof Num !== 'undefined' ? Num : _c$600.Num], typeof Num !== 'undefined' ? Num : _c$600.Num).proj(_c$600.Blame.create('mkSort', 'function mkSort', '(calling context for mkSort)', 20))(function mkSort$2(x) {
        var arr = [];
        for (var i = 0; i < x; i++) {
            arr.push(i);
        }
